...
# monta en un directorio temporal un consumidor Composer (bgm/core por
# versión, repositorio path) y un consumidor Vite (@bgm/ui y @bgm/admin-kit
...
echo "== Motor: $MOTOR_DIR (bgm/core $VERSION)"
echo "== Consumidores en: $WORK"

# ---------------------------------------------------------------- Composer --
echo
echo "== 1/2 · Composer: instalar bgm/core $VERSION desde un proyecto externo"
...
cat > "$WORK/consumidor-api/composer.json" <<JSON
{
    ...
    "repositories": {
        "bgm-core": { "type": "path", "url": "$MOTOR_DIR/packages/core" }
    },
...
$clases = [
    "Bgm\\Core\\MotorServiceProvider",
    "Bgm\\Core\\Auth\\MotorAuth",
    "Bgm\\Core\\Pdf\\PdfService",
    "Bgm\\Core\\Content\\Fields\\Field",
];
foreach ($clases as $c) {
    if (!class_exists($c)) { fwrite(STDERR, "FALTA $c\n"); exit(1); }
}
echo "   bgm/core autocarga OK (" . count($clases) . " clases)\n";
' "$WORK"

INSTALADA="$(cd "$WORK/consumidor-api" && composer show bgm/core --format=json | php -r 'echo json_decode(stream_get_contents(STDIN))->versions[0];')"
echo "   versión instalada: $INSTALADA"

# --------------------------------------------------------------------- npm --
echo
echo "== 2/2 · Vite: compilar una app externa con @bgm/ui y @bgm/admin-kit"
...
cat > "$APP/package.json" <<JSON
{
  ...
  "dependencies": {
    "@bgm/ui": "file:$MOTOR_DIR/packages/ui",
    "@bgm/admin-kit": "file:$MOTOR_DIR/packages/admin-kit",
    "@lucide/vue": "^1.0.0",
    "vue": "^3.5.38",
    "vue-router": "^5.0.0"
  },
  ...
  css: {
    preprocessorOptions: {
      scss: {
        ...
        loadPaths: [
          fileURLToPath(new URL('./node_modules/@bgm/ui/scss', import.meta.url)),
        ],
      },
    },
  },
...
import { BaseButton, useHead } from '@bgm/ui'
import { EmptyState } from '@bgm/admin-kit'
import './estilos.scss'
