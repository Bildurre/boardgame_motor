...
const props = defineProps<{ modelValue: boolean; mode: 'create' | 'edit'; targetSlug?: string | null }>()
...
const form = reactive<{
  name: Record<string, string>; description: Record<string, string>; ability: Record<string, string>
  power: number; prestige: number; intrigue: number; money: number; is_published: boolean
}>({ name: {}, description: {}, ability: {}, power: 0, prestige: 0, intrigue: 0, money: 0, is_published: false })

const title = computed(() => (props.mode === 'create' ? t('characters.new') : t('characters.edit')))
const iconList = computed(() => icons.icons.filter((i) => i.url).map((i) => ({ name: i.name, url: i.url as string })))
// Coste = suma de las cuatro estadísticas; defensa = coste (solo lectura).
const cost = computed(() => (Number(form.power) || 0) + (Number(form.prestige) || 0) + (Number(form.intrigue) || 0) + (Number(form.money) || 0))
const hasName = () => Object.values(form.name).some((v) => v && v.trim() !== '')

function reset() {
  form.name = {}; form.description = {}; form.ability = {}
  form.power = 0; form.prestige = 0; form.intrigue = 0; form.money = 0; form.is_published = false
  image.value = null; currentImage.value = null; clearErrors()
}
...
watch(() => props.modelValue, async (open) => {
  ...
  if (props.mode === 'edit' && props.targetSlug) {
    try {
      ...
      form.name = c.name ?? {}; form.description = c.description ?? {}; form.ability = c.ability ?? {}
      form.power = c.power ?? 0; form.prestige = c.prestige ?? 0; form.intrigue = c.intrigue ?? 0; form.money = c.money ?? 0
      form.is_published = !!c.is_published; currentImage.value = c.image ?? null
    } catch {
      toast.danger(t('characters.toast.saveError')); emit('update:modelValue', false)
    }
  }
})
...
async function submit() {
  clearErrors()
  if (!hasName()) { errors.name = t('common.required'); return }
  saving.value = true
  try {
    if (props.mode === 'edit' && props.targetSlug) {
      await updateForm(props.targetSlug, toFormData()); toast.success(t('characters.toast.updated'))
    } else {
      await createForm(toFormData()); toast.success(t('characters.toast.created'))
    }
    emit('saved'); emit('update:modelValue', false)
  } catch (e) {
    mapServerErrors(e); toast.danger(t('characters.toast.saveError'))
...
<template>
  <EditModal
    ...
    @submit="submit"
  >
    <TranslatableInput v-model="form.name" :locales="locales.locales" :label="t('characters.fields.name')" required :error="errors.name" />
    <TranslatableInput v-model="form.description" :locales="locales.locales" :label="t('characters.fields.description')" type="wysiwyg" :icons="iconList" />
    ...
    <p class="stats-derived">
      ...
      <span>{{ t('characters.fields.defense') }}: <strong>{{ cost }}</strong></span>
    </p>
