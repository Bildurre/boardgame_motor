...
async function load(page = 1) { await list({ search: search.value, status: status.value, page }) }
watch(status, () => load(1))
let timer: ReturnType<typeof setTimeout> | null = null
watch(search, () => { if (timer) clearTimeout(timer); timer = setTimeout(() => load(1), 250) })

const formOpen = ref(false)
const formMode = ref<'create' | 'edit'>('create')
const formSlug = ref<string | null>(null)
function openCreate() { formMode.value = 'create'; formSlug.value = null; formOpen.value = true }
function edit(item: any) { formMode.value = 'edit'; formSlug.value = slugFor(item); formOpen.value = true }
function goSingle(item: any) { router.push({ name: 'character-single', params: { slug: slugFor(item) } }) }
function onSaved() { load(meta.value?.current_page ?? 1) }

async function togglePublish(item: any) {
  await action(slugFor(item), 'toggle-published')
  toast.success(item.is_published ? t('characters.toast.unpublished') : t('characters.toast.published'))
  load(meta.value?.current_page ?? 1)
}
async function del(item: any) {
  const ok = await confirm({ title: t('characters.confirmDelete.title'), message: t('characters.confirmDelete.message', { name: tr(item.name) }), confirmLabel: t('houses.actions.delete'), variant: 'danger' })
  if (!ok) return
  await remove(slugFor(item)); toast.success(t('characters.toast.deleted')); load(meta.value?.current_page ?? 1)
}
async function restore(item: any) {
  await action(item.id, 'restore'); toast.success(t('characters.toast.restored')); load(meta.value?.current_page ?? 1)
}
async function forceDelete(item: any) {
  const ok = await confirm({ title: t('characters.confirmForceDelete.title'), message: t('characters.confirmForceDelete.message', { name: tr(item.name) }), confirmLabel: t('houses.actions.forceDelete'), variant: 'danger' })
  if (!ok) return
  await api.delete(`/admin/characters/${item.id}/force`); toast.success(t('characters.toast.forceDeleted')); load(meta.value?.current_page ?? 1)
}

onMounted(async () => { await locales.load(); load() })
</script>
...
<template>
  <div class="characters">
    ...
    <BaseGrid v-else preset="cards" gap="md">
      ...
      >
        ...
        <template #actions>
          ...
          <template v-else>
            <IconButton variant="success" :title="t('houses.actions.edit')" @click="edit(item)"><SquarePen :size="18" /></IconButton>
            <IconButton :variant="item.is_published ? 'warning' : 'info'" :title="item.is_published ? t('houses.actions.unpublish') : t('houses.actions.publish')" @click="togglePublish(item)">
        ...
        <template #badges>
          <span v-if="item.deleted_at" class="chip chip--trashed">{{ t('characters.state.trashed') }}</span>
          <span v-else-if="item.is_published" class="chip chip--pub">{{ t('characters.state.published') }}</span>
    ...
    <CharacterFormModal v-model="formOpen" :mode="formMode" :target-slug="formSlug" @saved="onSaved" />
  </div>
</template>
