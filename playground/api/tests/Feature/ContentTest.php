...
it('la paleta lista los tipos del motor y los del juego con su esquema', function () {
    $response = $this->actingAs(motorUser('admin'))->getJson('/api/admin/block-types')
        ->assertOk()
        ->assertJsonCount(12, 'data'); // 8 presentación + 1 con-datos (motor) + 3 con-datos (juego)

    $keys = collect($response->json('data'))->pluck('key');
    expect($keys)->toContain('header', 'text', 'text-card', 'quote', 'cta', 'index', 'faq', 'related', 'tabs', 'characters-grid', 'houses-schemes', 'featured-house');
...
it('el fondo de bloque acepta hex y presets dinámicos del tema (token:*)', function () {
    ...
    // cuatro velos del fondo de página + el acento translúcido. Los
    // retirados (surface*/accent-500 de 0.5.8, neutral* de 0.5.10) ya NO
    // viajan como preset…
    ...
    $this->actingAs($admin)->postJson("/api/admin/pages/{$page->id}/blocks", [
        'type' => 'header',
        'settings' => ['title' => ['es' => 'Legacy gris'], 'background' => 'token:neutral'],
    ])->assertCreated();
